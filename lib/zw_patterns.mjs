...
// Constants inline (no external dep needed)
...
export function detectPatterns(palaces) {
  if (!palaces) return [];
  const ming = palaces.find(p => p.name === '命宫');
  if (!ming) return [];
...
export function getSanFangAnalysis(palaces) {
  if (!palaces) return '';
  const ming = palaces.find(p => p.name === '命宫');
...
export function getStarBrightnessTable(palaces) {
  let r = `**星曜亮度**\n`;
...
export function getCareerSuggestion(palaces) {
  const ming = palaces.find(p => p.name === '命宫');
