...
export function genLiuYao() {
  return genLiuYaoData().text;
}

export function genLiuYaoData() {
  // 使用数字起卦（3个随机数1-10）
  ...
  try {
    const now = new Date();
    benGua = decodePan(yaoStr, { year: now.getFullYear(), month: now.getMonth() + 1, day: now.getDate(), hour: now.getHours() });
  } catch(e) {
  ...
  return { text: r, data: { guaName, moving, yaoStr, nums } };
}
