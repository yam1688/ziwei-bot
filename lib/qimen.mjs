...
export function genQiMenData(year, month, day, hour, method = 'chaiBu') {
  ...
  try {
    const baMenMean = getBaMenMeanings(baMen);
    const jiuXingMean = getJiuXingMeanings(jiuXing);
    if (baMenMean) r += `\n📖 **八门吉凶**\n${baMenMean}\n`;
    if (jiuXingMean) r += `📖 **九星吉凶**\n${jiuXingMean}\n`;
  } catch(e) { /* skip extras on error */ }
  r += `\n📖 **解盘**\n`;
  r += interpQiMen(obj);
  return { text: r, data: JSON.parse(JSON.stringify(obj)) };
}
